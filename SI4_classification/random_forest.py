...
class RandomForest:
    ...
    def bagging(self, X, y):
        ...
        num_selected = int(self.params["bagging_fraction"] * num_samples)

        indices = np.random.choice(num_samples, size=num_selected, replace=True) # losowanie ze zwracaniem
        unique_indices = np.unique(indices)
        X_selected = X[unique_indices]
        y_selected = y[unique_indices]
