...
def zad2_two_layer_net(student_id):
    ...
    class SimpleTwoLayerNetwork:
        ...
        def forward(self, x_data):
            hidden = self.hidden_layer.forward(x_data)
    ...
    model.hidden_layer.W[:, 0] = [1.0, 0.5]      # wagi neuronu h1
    model.hidden_layer.W[:, 1] = [-1.0, -0.3]      # wagi neuronu h2
    model.hidden_layer.b[:] =  [-0.2, 1.1]         # biasy neuronów h1 i h2
    model.output_layer.W[:, 0] = [1.0, 1.0]      # wagi neuronu wyjściowego
    model.output_layer.b[:] = [-1.4]         # bias neuronu wyjściowego
...
if __name__ == '__main__':
    ...
    student_id = 193504         # Twój numer indeksu, np. 102247
