...
class Node:
    ...
    def gini(self, y):
        ...
        if total == 0:
            return 0
        gini = 1 - ((pos / total) ** 2) - ((neg / total) ** 2)
        return gini

    def gini_gain_func(self, y, y_left, y_right):
        gini_gain = 1 - (len(y_left)/len(y)) * self.gini(y_left) - (len(y_right)/len(y)) * self.gini(y_right)
        return gini_gain
    ...
    def gini_best_score(self, y, possible_splits):
        ...
        for split in possible_splits:
            y_left = y[:split+1]
            y_right = y[split+1:]
            gini_gain = self.gini_gain_func(y, y_left, y_right)
    ...
    def find_best_split(self, X, y, feature_subset, params):
        best_gain = -np.inf
        best_split = None
        selected_features = list(range(X.shape[1]))
        # TODO implement feature selection
        if params["type"] == 1:
            selected_features = np.random.choice(range(X.shape[1]), size=X.shape[1] - feature_subset, replace=False)
    ...
    def train(self, X, y, params):
        ...
        self.feature_idx, self.feature_value = self.find_best_split(X, y, params["feature_subset"], params)
        if self.feature_idx is None:
            return True
